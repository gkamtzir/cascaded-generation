    ...
    if args.results_path is not None:
        ...
        if args.ngpus == 1:
            ...
            for rank in range(args.ngpus):
                p = Process(target=_main, args=(args, sys.stdout, rank, world_size, backend, master_addr, master_port))
                p.start()
                processes.append(p)
            for p in processes:
                p.join()
...
def _main(args, output_file, rank=0, world_size=1, backend='NCCL', master_addr='127.0.0.1', master_port='29500'):
    if world_size > 1:
    ...
    with torch.cuda.device(rank):
        ...
        for sample in progress:
            ...
            gen_timer.start()
            hypos = task.inference_step(generator, models, sample, prefix_tokens, topk=args.topk, D=args.D, rounds=args.rounds, rank=rank, ngpus=world_size)
